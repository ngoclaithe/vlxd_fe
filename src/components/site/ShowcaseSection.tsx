  ...
  return (
    <section id="du-an" className="showcase-section mx-auto container-max px-4 py-16 md:px-6">
      <div className="mb-8 text-center">
        <h2 className="section-title text-3xl font-bold tracking-tight text-gray-900 dark:text-white">Dự án tiêu biểu</h2>
        <p className="mt-2 text-sm text-muted">Một vài công trình mà NovaBuild đã đồng hành</p>
      </div>
        ...
        {projects.map((p) => (
          <article key={p.name} className="project-card card overflow-hidden p-6">
            ...
            <div className="flex items-center justify-between">
              <span className="tag inline-flex items-center rounded-full bg-white/90 px-3 py-1 text-xs font-medium text-gray-800">{p.tag}</span>
