...
export default function Header() {
  ...
  useEffect(() => {
    function onClickOutside(e: MouseEvent) {
      ...
      if (brandsRef.current && !brandsRef.current.contains(target)) {
        setBrandsOpen(false);
      }
    }
  ...
  return (
    <header className="site-header sticky top-0 z-50 w-full bg-transparent backdrop-blur-md border-b border-black/5 dark:border-white/10">
      <div className="mx-auto flex max-w-screen-xl items-center justify-between px-4 py-4 md:px-6">
        ...
        <nav className="main-nav hidden items-center gap-8 md:flex">
          ...
          <div ref={productsRef} className="relative">
            <div className="inline-flex items-center gap-2">
              <Link href="/products" className="nav-link text-sm font-medium tracking-wide text-white/90 hover:text-white">
                Sản phẩm
              </Link>
              <button
                type="button"
            ...
            <div
              id="products-menu"
              className={`${productsOpen ? "block" : "hidden"} absolute left-0 top-full mt-3 w-72 rounded-lg bg-white text-black border border-black/10 p-3 shadow-2xl z-50`}
            >
                ...
                {[
                  ...
                  <li key={c}>
                    <Link
                      ...
                      className="block rounded px-3 py-2 text-[clamp(12px,1.6vmin,15px)] text-black hover:bg-black/5"
          ...
          <div ref={brandsRef} className="relative">
            <div className="inline-flex items-center gap-2">
              <Link href="#brands" className="nav-link text-sm font-medium tracking-wide text-white/90 hover:text-white">
                Thương hiệu
              </Link>
              <button
                type="button"
            ...
            <div
              id="brands-menu"
              className={`${brandsOpen ? "block" : "hidden"} absolute left-0 top-full mt-3 w-80 rounded-lg bg-white text-black border border-black/10 p-4 shadow-2xl z-50`}
            >
        ...
        <button
          aria-label="Mở menu"
          className="menu-toggle inline-flex h-10 w-10 items-center justify-center rounded-lg border border-transparent bg-white/5 text-white md:hidden"
          onClick={() => setOpen((s) => !s)}
        >
          ...
          <div className="flex flex-col gap-3">
              ...
              <div className="grid gap-1 px-2">
                {[
                  ...
                  <Link
                    key={c}
                    href={`/products?category=${encodeURIComponent(c)}`}
                    className="block rounded-md px-3 py-2 text-sm font-medium text-white hover:bg-white/6"
            ...
            {NAV_LINKS.filter((l) => l.label !== "Sản phẩm" && l.label !== "Thương hiệu").map((l) => (
              <Link
                key={l.href}
                href={l.href}
                className="nav-link block rounded-md px-3 py-2 text-sm font-medium text-white hover:bg-white/6"
                onClick={() => setOpen(false)}
              >
                {l.label}
              </Link>
